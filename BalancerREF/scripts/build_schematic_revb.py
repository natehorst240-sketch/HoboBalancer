...
def place(ref,lib,x,y,angle=0,value=None,fp=None,field=None,mpn=None):
 ...
 inst=node('symbol',node('lib_id',libid),node('at',x,y,angle),node('unit',1),node('in_bom',S('no' if ref.startswith('#') else 'yes')),node('on_board',S('no' if ref.startswith('#') else 'yes')),node('dnp',S('no')),node('uuid',uid()))
 ...
 for k,v in [('Reference',ref),('Value',value if value is not None else getprop(sym,'Value')),('Footprint',fp if fp is not None else getprop(sym,'Footprint')),('Datasheet',getprop(sym,'Datasheet')),('MPN',mpn or value or getprop(sym,'Value'))]:
  ...
  inst.append(p)
 inst.append(node('instances',node('project','BalancerREF',node('path','/'+sheetid,node('reference',ref),node('unit',1)))))
...
box('ACCELEROMETER - INTERNAL XYZ',10.16,266.7,172.72,114.3)
box('MAIN ROTOR MAG TACH',187.96,266.7,193.04,114.3)
box('TAIL ROTOR OPTICAL TACH',386.08,266.7,193.04,114.3)
box('USER INPUTS / STATUS',10.16,137.16,165.1,121.92)
text('BalancerREF - rotor vibration reference puck',12.7,7.62,3)
text('Single sheet | Rev B | KiCad 9 | schematic review before PCB',345.44,9,1.7)
...
gpio={8:'ACCEL_SDA',9:'ACCEL_SCL',10:'ACCEL_DRDY',11:'MAG_TACH',12:'OPT_TACH',13:'OPT_LED_EN',14:'ACQUIRE_BUTTON',15:'STATUS_LED',16:'SUPPLY_SENSE'}
...
text('Active-low buttons; debounce in firmware.\nBOOT held low during reset enters download.\nSupply ADC: SYS_SW / 2; dead when OFF.\nOne green status LED; patterns set by firmware.',101.6,220.98,1.2)

# Accelerometer: physical pull-ups and explicit supply/CS/SA0/GND/RES wiring.
place('U2','LIS2DW12TR',99.06,325.12,field=(99.06,304.8))
wire((25.4,292.1),(154.94,292.1));rail('+3V3',25.4,292.1)
for n in [9,10]:route('U2',n,(p('U2',n)[0],292.1))
...
rcl=R('4.7k',30.48,304.8);rda=R('4.7k',53.34,304.8)
for r in [rcl,rda]:route(r,1,(p(r,1)[0],292.1))
route(rcl,2,(30.48,317.5),p('U2',1));route(rda,2,(53.34,322.58),p('U2',4))
wire((20.32,317.5),(30.48,317.5));label('ACCEL_SCL',20.32,317.5,180)
wire((20.32,322.58),(53.34,322.58));label('ACCEL_SDA',20.32,322.58,180)
route('U2',2,(73.66,327.66),(73.66,292.1))
route('U2',3,(73.66,332.74),(73.66,350.52));
for n in [6,7,8]:route('U2',n,(p('U2',n)[0],350.52))
wire((73.66,350.52),(104.14,350.52));gnd(99.06,350.52)
route('U2',12,(151.13,317.5));label('ACCEL_DRDY',151.13,317.5)
tp('TP10','TP_ACCEL_DRDY',124.46,312.42,(124.46,317.5))
nc('U2',11,'INT2 unused; keep in interrupt output mode');nc('U2',5,'Internally unconnected pad')
text('CS = VDDIO; SA0 = GND -> 7-bit address 0x18.\nRES pad 7 MUST be GND. All three axes available.\nFirmware: high-performance, low-noise, 800/1600 Hz;\nINT1 data-ready timestamps; select bandwidth for RPM.\nPlace near rigid mount; vertical axis set by orientation.',17.78,358.14,1.2)
...
# IR emitter on +3V3, MOSFET low-side switch with hardware default OFF.
wire((398.78,292.1),(571.5,292.1));rail('+3V3',398.78,292.1)
re=R('100R',406.4,302.26);route(re,1,(406.4,292.1))
place('D1',('Device','LED'),406.4,317.5,90,'LTE-4208 940nm',fp='',field=(414.02,312.42));route(re,2,p('D1',2))
...
# Phototransistor collector is raw signal; two resistors plus trim set ambient threshold.
place('Q2',('Device','Q_Photo_NPN_EC'),449.58,340.36,value='LTR-4206E',fp='',field=(448.31,346.71))
rload=R('10k',452.12,304.8);route(rload,1,(452.12,292.1));route(rload,2,(452.12,320.04),p('Q2',2));route('Q2',1,(452.12,363.22));gnd(452.12,363.22)
cf=C('100p C0G',469.9,340.36);route(cf,1,(469.9,320.04),(452.12,320.04));route(cf,2,(469.9,363.22),(452.12,363.22))
tp('TP8','TP_OPT_RAW',457.2,314.96,(457.2,320.04))
# Adjustable 0.8..2.5V threshold; positive feedback creates Schmitt hysteresis.
rvt=R('4.7k',495.3,304.8);rvb=R('4.7k',495.3,345.44)
place('RV1',('Device','R_Potentiometer'),495.3,325.12,value='10k trim',fp='Potentiometer_SMD:Potentiometer_Bourns_3314G_Vertical',field=(483.87,327.66))
route(rvt,1,(495.3,292.1));route(rvt,2,p('RV1',1));route('RV1',3,p(rvb,1));route(rvb,2,(495.3,363.22));gnd(495.3,363.22)
place('U4','TS3021IYLT',538.48,322.58,field=(539.75,305.8))
route('Q2',2,(518.16,335.28),(518.16,p('U4',4)[1]),p('U4',4))
route('RV1',2,(510.54,325.12),(510.54,p('U4',3)[1]),p('U4',3))
route('U4',5,(p('U4',5)[0],292.1));route('U4',2,(p('U4',2)[0],363.22));gnd(p('U4',2)[0],363.22)
route('U4',1,(568.96,322.58));label('OPT_TACH',568.96,322.58)
tp('TP9','TP_OPT_TACH',563.88,317.5,(563.88,322.58))
rhys=R('470k',548.64,345.44,90);route(rhys,1,(568.96,345.44),(568.96,322.58));route(rhys,2,(510.54,345.44),(510.54,325.12))
cv=caprail('100n',571.5,304.8,292.1,312.42);gnd(571.5,312.42)
text('D1 + Q2 face LEFT edge; formed leads, optical baffle/shroud.\nEmitter ~20mA; HIGH=TR, LOW=MR / default OFF.\nRV1: dark/reflection midpoint; ~34mV hysteresis at mid-trim.',393.7,368.3,1.12)

text('MR-VERT: external magnetic pickup; optical LED OFF.\nTR-VERT: onboard reflective optical tach; LED ON.\nBoth use internal XYZ MEMS and BLE to phone.\n\nReport approximate RPM, 1/rev magnitude, phase,\nquality/stability and run-to-run trends.\n\nREFERENCE / TROUBLESHOOTING ONLY.\nMaintenance balancing remains with calibrated\nMicroVib / DynaVibe equipment.',439.42,35.56,1.75)
text('Mechanical intent only: future PCB ~50 x 40 mm.\nNo PCB generated. Keep antenna area clear.\nRigid mount near MEMS; baffled optical window on LEFT.\nValidate phase delay and tach SNR on bench before use.',439.42,99.06,1.45)
...
# ST application figure 6 also calls for local bulk capacitance at VDD.
...
for inst in instances:
 ...
 for pr in children(inst,'property'):
  ...
  if ref=='C19' and pr[1] in ['Reference','Value']:one(pr,'at')[1]=558.8
  if ref=='C20' and pr[1] in ['Reference','Value']:
   one(pr,'at')[1]=160.02
...
(ROOT/'sym-lib-table').write_text('(sym_lib_table (version 7) (lib (name "BalancerREF")(type "KiCad")(uri "${KIPRJMOD}/BalancerREF.kicad_sym")(options "")(descr "Verified symbols missing from KiCad 9")))\n')
